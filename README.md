...
- **Dual-Path Text Embedding & Ingestion Pipelines**:
  ...
  - `notebooks/spark_ingestion_pipeline.ipynb` & `src/spark_pipeline.py`: Distributed PySpark ingestion and vector embedding job. Runs on serverless Databricks compute, utilizing `clean_papers_df` DataFrame transformations (deduplication, abstract length filtering, citation bucketing, prompt formatting) and `mapInPandas` for per-partition model loading (`sentence-transformers/all-MiniLM-L6-v2`, $N=384$ vector dimensions) and atomic Lakebase batch upserts.
